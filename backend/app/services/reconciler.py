...
class Reconciler:
    ...
    def reconcile(self, message: NormalizedEmail, decision: RoutingDecision, run_id: str) -> str:
        with self.store.thread_lock(message.email.thread_id):
            ...
            remote_matches = self.store.list_tasks(LOCKED_CANDIDATE_ID, thread_id=message.email.thread_id)
            if len(remote_matches) > 1:
                raise AppError("task_thread_conflict", "multiple tasks exist for one ingested thread", status_code=409)
            ...
            if mapped_id and remote is None:
                mapped = self.store.get_task(mapped_id)
                if mapped is None:
                    raise AppError("mapped_task_missing", "previously mapped task is missing", status_code=500)
                remote = mapped
            ...
            if message.email.message_index > 0 and not has_local_predecessor and not remote:
                raise AppError("orphan_reply", "reply has no stored predecessor or task", status_code=409)

            if remote is None:
                event = self._event(message, "create", None, decision.task.model_dump(mode="json"), None, status="pending")
                remote = self.store.create_task(decision.task)
            ...
            if not task_id:
                raise AppError("invalid_task_record", "task record has no task_id", status_code=500)
            after = self.store.patch_task(task_id, patch)
